...
You will need the following libraries: MSnSet.utils, plotly, tidyverse, and TopPICR.
...
The files needed to run the TopPICR pipeline can be cloned from this GitHub
[repo](https://github.com/PNNL-Comp-Mass-Spec/TopPICR_reproducible_code_for_JPR).


Create a path to the directory containing the TopPIC output and fasta files.
NOTE: Be sure to update the path to the cloned repo.

```{r}
the_path <- "/Users/mart077/Documents/data/cptac/"
```
...
ms2_mp <- map_proteoform(
  ...
  fst_name = "ID_008269_B09DF3DE.fasta.gz"
)
...
```{r,results='markup'}
# Count unique PrSMs, proteoforms, and genes -----------------------------------
...
# Count the number of unique clean sequences.
ms2_mp %>%
  distinct(cleanSeq) %>%
  nrow()
...
# respectively.
compute_fdr(filter(ms2_clu, cluster != 0))
compute_fdr(ms2_grp)
...
The following code calculates the proportion of missing data with and without the MBR step.
...
sum(is.na(exprs(m_no_mbr))) / prod(dim(exprs(m_no_mbr))) * 100

# Compute the proportion of missing data WITH the MBR step.
sum(is.na(exprs(m0))) / prod(dim(exprs(m0))) * 100
```
